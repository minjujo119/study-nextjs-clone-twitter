...
export default function CommentItem({
  ...
  // 삭제 버튼 클릭 시 동작
  const onClick = async () => await deleteComments(id);

  return (
    <div className="flex gap-5 py-4 justify-between items-start border-t border-t-[var(--border-color)]">
      ...
      <button
        onClick={onClick}
        className="btn btn-sm text-red-500 border-red-500"
      >
        삭제
      </button>
    </div>
